...
function renderSongs() {
  ...
  const filtered = allSongs.filter((song) => {
    const haystack = normalize([song.title, song.name, song.artist, song.tone, song.key, song.category].join(" "));
  ...
  songsGrid.innerHTML = filtered.map((song) => {
    ...
    return `<article class="song-card"><h3>${title}</h3><p>${type}</p>${tone ? `<p><strong>Tono:</strong> ${tone}</p>` : ""}<a class="song-btn" href="cancion.html?id=${id}">Ver canción</a></article>`;
  }).join("");
}
