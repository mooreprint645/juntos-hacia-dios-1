...
function show(on) { ["#adminPanel", "#relationsPanel", "#recentPanel"].forEach((id) => $(id)?.classList.toggle("hidden", !on)); $("#loginSection")?.classList.toggle("hidden", on); }
...
async function hasSession() { const { data } = await client.auth.getSession(); const ok = Boolean(data?.session); show(ok); if (ok) await refresh(); return ok; }
async function add(table, raw, okText) { if (!await hasSession()) { note("#adminMessage", "Primero entra al panel."); return false; } const { error } = await client.from(table).insert([payload(table, raw)]); if (error) { note("#adminMessage", `Error: ${error.message}`); return false; } note("#adminMessage", okText); await refresh(); return true; }
function bind() {
  $("#loginForm")?.addEventListener("submit", async (e) => { e.preventDefault(); const f = formData(e.currentTarget); note("#loginMessage", "Revisando acceso..."); const { error } = await client.auth.signInWithOtp({ email: f.email, options: { emailRedirectTo: window.location.href } }); note("#loginMessage", error ? error.message : "Revisa tu correo para entrar al panel."); });
