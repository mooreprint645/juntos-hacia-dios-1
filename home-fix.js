(() => {
  ...
  JHD.loadHome = async () => {
    ...
    const songsTask = JHD.sb.from("songs").select("id,title,slug,song_type,tone,difficulty").order("title", { ascending: true }).limit(6);
    const artistsTask = JHD.sb.from("artists").select("id,name,slug,description,artist_type").order("name", { ascending: true }).limit(6);
    const categoriesTask = JHD.sb.from("categories").select("id,name,slug,parent_id,song_type,sort_order").order("sort_order", { ascending: true }).order("name", { ascending: true });
    const [songsRes, artistsRes, categoriesRes] = await Promise.all([songsTask, artistsTask, categoriesTask]);
