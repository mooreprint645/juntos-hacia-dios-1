...
async function saveDonationSettings(row) {
  const current = await AdminExtra.from("donation_settings").select("id").order("updated_at", { ascending: false }).limit(1).maybeSingle();
  if (current.error) return current.error;
  const result = current.data ? await AdminExtra.from("donation_settings").update(row).eq("id", current.data.id) : await AdminExtra.from("donation_settings").insert([row]);
  return result.error;
}
async function save(table, form, message) {
  ...
  const error = table === "donation_settings" ? await saveDonationSettings(row) : (await AdminExtra.from(table).insert([row])).error;
